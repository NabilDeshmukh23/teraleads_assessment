...
const fetchPatients = async (query = '') => {
  setLoading(true);
  try {
    const response = await api.get(`/patients/list?page=1&search=${query}`);
...
useEffect(() => {
  ...
  const calculateInsights = () => {
    if (patients.length === 0) return { total: 0, recentlyAdded: 0 };
    const oneDayAgo = new Date(Date.now() - 24 * 60 * 60 * 1000);
    const recentlyAdded = patients.filter(p => new Date(p.createdAt) > oneDayAgo).length;
    return { total: patients.length, recentlyAdded };
  };

  const { total, recentlyAdded } = calculateInsights();
      ...
      <main className="max-w-6xl mx-auto px-6 py-10">
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-10">
          <div className="bg-white p-6 rounded-2xl border border-gray-200 shadow-sm flex items-center justify-between transition-all hover:shadow-md">
            <div><p className="text-xs font-bold text-gray-400  tracking-widest">TOTAL PATIENTS</p><p className="text-4xl font-black text-slate-900">{loading ? '...' : total}</p></div>
            <Activity className="text-blue-500" size={40} />
          </div>
          <div className="bg-white p-6 rounded-2xl border border-gray-200 shadow-sm flex items-center justify-between transition-all hover:shadow-md">
            <div><p className="text-xs font-bold text-gray-400  tracking-widest">NEW (24H)</p><p className="text-4xl font-black text-slate-900">+{loading ? '..' : recentlyAdded}</p></div>
          ...
          <div className="p-8 border-b border-gray-100 flex flex-col md:flex-row justify-between items-center gap-4 bg-gray-50/30">
            <div className="relative w-full md:w-96">
              ...
              <input 
                ...
                className="w-full pl-12 pr-4 py-3 bg-white border border-gray-200 rounded-2xl outline-none focus:ring-4 focus:ring-blue-50 transition text-sm  font-bold tracking-tight"
              ...
              <tbody className="divide-y divide-gray-100 font-bold text-sm">
                {loading ? (
                  ...
                  <tr key={p.id} className={`transition group ${selectedPatient?.id === p.id ? 'bg-blue-50/50' : 'hover:bg-blue-50/30'}`}>
                    <td className="px-8 py-6 text-slate-900 uppercase flex items-center gap-2">
                      {p.name}
                    ...
                    <td className="px-8 py-6 text-right">
                      <div className="flex justify-end gap-4 text-gray-300">
                        <button 
                          ...
                          className={`${selectedPatient?.id === p.id ? 'text-blue-600' : 'hover:text-blue-600'} transition active:scale-90`} 
                          title="START AI CHAT"
                        >
  ...
  <form onSubmit={handleSubmit} className="space-y-5">
    ...
    <div className="space-y-2">
      ...
      <textarea 
        className="w-full p-4 bg-gray-50 border border-gray-200 rounded-2xl outline-none focus:border-blue-500 transition h-28 resize-none font-bold text-xs leading-relaxed" 
    ...
    <button 
      type="submit" 
      disabled={isSubmitting} 
      className="w-full bg-blue-600 text-white py-5 uppercase rounded-2xl font-black hover:bg-blue-700 shadow-xl transition mt-4 text-xs flex justify-center items-center gap-2"
    >
